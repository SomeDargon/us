...
<h4>用户<a href="${ctx }/admin/profile/avatar" class="btn-link btn-sm pull-right"><span class="glyphicon glyphicon-plus-sign"></span>添加头像</a><a href="${ctx }/admin/user/form" class="btn-link btn-sm pull-right"><span class="glyphicon glyphicon-plus-sign"></span>添加用户</a></h4>
...
<table id="user-table" class="table table-striped table-bordered table-hover">
    ...
    {{each result as value i}}
		<tr>
			...
			<td>{{ value.email }}</td>
			<td>{{ value.phone }}</td>
			<td>{{if value.role != null }}{{ value.role.name }}{{/if}}</td>
			<td><c:forEach items="${fns:getDictList('user_schedule')}" var="dict">
				{{if value.schedule == '${dict.value}'}}${dict.label}{{/if}}
				</c:forEach>
			</td>
            <td>
...
<c:if test="${env == 'pro' }">
    requirejs(['${ctx}/assets/js/app/admin/user-manage-page.js']);
</c:if>
<c:if test="${env != 'pro' }">
    requirejs(['${ctx}/assets/js/app/admin/user-manage-page.js']);
</c:if>
