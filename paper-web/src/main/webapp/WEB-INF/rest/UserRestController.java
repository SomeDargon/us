...
public class UserRestController extends BaseController{
    ...
    public ResponseEntity<?> list(@RequestParam(value = "p", defaultValue = "1") int pageNo,
        ...
        page = systemService.findUsers(page, user, orderName, order);
        return new ResponseEntity(page, HttpStatus.OK);
    }

	@RequestMapping(value="/save", method = RequestMethod.POST)
	public ResponseEntity<?> apiUserAdd(@Valid User user) throws InvocationTargetException, IllegalAccessException{
        Map<String, Object> map = Maps.newHashMap();

        if (StringUtils.isBlank(user.getPhone())) user.setPhone(null);
        User oldUser = new User();
        if (user.getId() != null) {
            oldUser = systemService.getUser(user.getId());
        }

        BeanUtilsBean bean = new BeanUtils();
        bean.copyProperties(oldUser, user);
